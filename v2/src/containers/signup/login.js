            ...
            <div className="login-wrapper">
                <div className="login-body">
                    ...
                    <div className="login-form">
                        <h3>您好，请先登录！</h3>
                        <select onChange={(e)=>{this.setState({id: e.target.value})}} className="select">
                            {this.props.users.get('items').map((item)=>(<option value={item.get('u_id')}>{item.get('u_name')}</option>))}
                        </select>
                        <div className="input">
